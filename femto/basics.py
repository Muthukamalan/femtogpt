...
class MoEFeedForward(nn.Module):
    def __init__(self, cfg:QwenSmolConfig):
        super().__init__()
        ...
        self.fc1 = nn.ModuleList([nn.Linear(cfg.emb_dim, cfg.moe_intermediate_size, bias=False)
                                  for _ in range(cfg.num_experts)])
        self.fc2 = nn.ModuleList([nn.Linear(cfg.emb_dim, cfg.moe_intermediate_size, bias=False)
                                  for _ in range(cfg.num_experts)])
        self.fc3 = nn.ModuleList([nn.Linear(cfg.moe_intermediate_size, cfg.emb_dim, bias=False)
                                  for _ in range(cfg.num_experts)])
    ...
    def forward(self, x:torch.Tensor):
        ...
        out_flat = torch.zeros(batch * seq_len, self.emb_dim, device=x.device)

        topk_indices_flat = topk_indices.reshape(-1, self.num_experts_per_tok)
        topk_probs_flat = topk_probs.reshape(-1, self.num_experts_per_tok).to(x.dtype)
